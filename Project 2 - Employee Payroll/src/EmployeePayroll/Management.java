...
public class Management extends Fulltime{
	...
	*/
	...
	public void calculatePayment() {
		...
		double managementRole = super.getProfile().getManagementRole();
		double payment;
		
		if(managementRole == MANAGER_ROLE) {
			
			payment = (super.getProfile().getAnnualSalary() / PAY_PERIODS_PER_YEAR)+ MANAGER_COMPENSATION;
			super.setPayment(payment);
		}	
			
		else if(managementRole == DEPARTMENT_HEAD_ROLE) {
			
			payment = (super.getProfile().getAnnualSalary() / PAY_PERIODS_PER_YEAR) + DEPARTMENT_HEAD_COMPENSATION;
			super.setPayment(payment);
		}
		
		else if(managementRole == DIRECTOR_ROLE) {
			
			payment = (super.getProfile().getAnnualSalary() / PAY_PERIODS_PER_YEAR) + DIRECTOR_COMPENSATION;
			super.setPayment(payment);
		}	
	}
			...
			return emp.getProfile().getName().equals(this.getProfile().getName()) 
					&& emp.getProfile().getDepartment().equals(this.getProfile().getDepartment()) 
						&& (emp.getProfile().getDateHired().compareTo(this.getProfile().getDateHired()) == 0)
							&& (emp.getProfile().getAnnualSalary()) == (this.getProfile().getAnnualSalary()) 
								&& (emp.getProfile().getManagementRole()) == (this.getProfile().getManagementRole());
		}
		return false;
	}
}
